...
| | | | | | \
|custom_speaker_id |1 |string |可选 |自定义音色代号； |\
| | | | |**使用说明：** |\
| | | | | |\
| | | | |1. 当使用该值自定义音色代号名称时，**speaker_id** 需要填写为固定值：**"custom_speaker_id"** |\
| | | | | |\
| | | | |```JSON |\
| | | | |{ |\
| | | | |    "speaker_id": "custom_speaker_id", // 固定值 |\
| | | | |    "custom_speaker_id": "custom_zh_xxx" // 客户自定义命名 |\
| | | | |} |\
| | | | |``` |\
| | | | | |\
| | | | | |\
...
| | | | |* 允许数字、字母、-、_ |\
| | | | |* 首位和末位字符不可为 -、_ |\
| | | | |* 在相同accountID维度下不可与已有 ID 重复，否则会报错 |\
| | | | |3. **该字段取值禁止规则：** |\
| | | | |* 不能和任何官方的精品音色相同，在正则表达式验证网站（[https://regex101.com/](https://regex101.com/)），如果匹配以下正则表达式则表示该音色 ID不可使用 |\
| | | | | |\
| | | | |```Go |\
...
| | | | |**注意事项：** |\
| | | | | |\
| | | | |1. 该字段表示后付费能力，开通与计费说明详细可以参见：[声音复刻下单及使用指南](https://www.volcengine.com/docs/6561/1167802?lang=zh) |\
| | | | |2. 该字段音色名不受限训练复刻次数，复刻的试听文本(demo_text)则会收取合成的费用 |\
| | | | |3. 该字段音色名只适用于合成v3 接口，首次调用v3 合成接口后，则表示该音色转正，转正之后则不可再使用该 ID 进行复刻，但是客户可以删除重新注册；新注册则会重新计费； |\
| | | | |4. 复刻但是没有调用合成 7天后自动删除，删除后释放的 ID 可以重复使用。 |
| | | | | | \
|audio |\
| |1 |\
| | |object |必须 |\
...
| |2 |bool |\
| | | |可选 |是否关闭音量归一化，默认值为 false，开启音量归一化，合成时是相对统一的音量，如果关闭音量归一化，合成出来的音量会和 prompt 更接近，和 prompt音频相似度也会更高。 |
| | | | | | \
|demo_text |\
| |2 |string |\
| | | |可选 |试听文本，长度在4和300字之间，如果指定了语种需要传入对应语种的文本，否则会合成失败。 |\
| | | | |**注意事项：** |\
| | | | | |\
| | | | |1. demo_text 文本越长，注册耗时越长，建议合理控制文本长度。 |
| | | | | | \
...
{
    "speaker_id": "custom_speaker_id", // 固定值
    "custom_speaker_id": "custom_zh_xxx", // 客户自定义音色名，有规则限制详见字段说明
    "audio": {
        "data": "base64编码后的音频",
        "format": "wav"
    },
...
|speaker_id |1 |string |必须 |唯一音色代号 |
| | | | | | \
|custom_speaker_id |1 |string |可选 |训练接口中自定义的音色代号 |\
| | | | |**使用说明：** |\
| | | | | |\
| | | | |1. 当使用该值时，**speaker_id** 需要填写为固定值：**"custom_speaker_id"** |
