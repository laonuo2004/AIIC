# ResearchMocker 演示脚本

目标时长：3 分钟以内。

## 开场：前 30 秒

立刻展示最强产品画面。

1. 打开 `http://115.190.120.206/`。
2. 登录测试账号，或快速注册。
3. 使用预填项目卡：
   - CS/AI 本科生。
   - 一个具体科研项目。
   - 目标方向。
   - 个人贡献。
   - 实验结果。
   - 担心被追问的点。
4. 上传 1 个 PDF/报告和 1 张结构图或结果图。
5. 开始模拟面试。
6. 展示 AI 提出基于项目材料的项目追问。
7. 故意给出空泛回答：`我们改了模型所以效果变好。`
8. 展示反馈：缺 baseline、缺指标、个人贡献不清、技术细节浅、回答节奏松。
9. 展示老师视角：为什么真实老师会继续追这个点。
10. 展示追问：`你怎么证明提升来自这个模块，而不是数据划分或训练设置？`

推荐输入在 `demo/`：

- 使用 `Self-introduction.md`、`Project or research experience.md`、`Target direction.md`、`Weak points.md` 填项目卡。
- 附件只放 1-2 个小文件，例如 `1-个人简历.pdf` 或 `2-个人自述.pdf`。
- `Interviewer.jpg` 和 `20260518_210026.mp3` 只用于数字人面试页。

建议旁白：

```text
ResearchMocker 是面向 CS/AI 保研科研面试的项目追问 AI 面试官。它不是泛聊天助手，而是用老师式追问找出空泛回答、缺失证据和个人贡献不清的问题，并告诉学生下次怎么答。
```

## 主流程

1. 展示面试资料：自我介绍、项目经历、面试方向、最担心的问题。
2. 展示附件上传：PDF、笔记、结构图或结果图。
3. 开始模拟面试。
4. AI 一次只问一个项目追问。
5. 用户用中文短答。
6. 展示逐题反馈：回答亮点、容易被追问的地方、老师视角、回答节奏、分数、下次怎么答。
7. 展示基于上一条回答生成的追问。
8. 结束面试。
9. 展示最终报告：总分、通过风险、最容易被追问的问题、明天前要练什么。

## 可选：数字人面试实验

只有页面状态足够稳定时再展示。

1. 打开数字人面试页。
2. 展示面试官图片和参考音频上传。
3. 简述目标：数字人状态、声音复刻、语音问答。
4. 如果接口不稳定，明确说明这是实验功能，然后回到稳定主流程。

建议旁白：

```text
主流程是稳定可用的模拟面试。数字人面试是下一层实验，用来靠近真实面试压力，但不影响主流程。
```

## 工程说明

保持简短：

- Backend: FastAPI, SQLite, HttpOnly sessions, pytest.
- Frontend: Next.js, TypeScript, responsive interview UI.
- LLM：只走后端 LiteLLM + OpenRouter。
- 模型：复杂分析和报告用 Plus，频繁问答用 Flash 或统一配置。
- 部署：Docker Compose + Nginx + 公网地址。
- 安全：密钥只在 `.env`，自动化测试 mock provider。

## 兜底方案

如果模型慢或偶发失败：

- 使用保存的历史面试记录。
- 展示提前生成的最终报告。
- 使用预填项目卡，减少现场输入。
- 附件只上传小 PDF 和一张图。

如果面对面模式不完整：

- 不要说成已完成。
- 简短展示设计，然后回到模拟面试。

## 收尾

结束语：

```text
ResearchMocker 的目标很窄：帮学生练会在追问下讲清自己的项目，把空泛回答改成有证据、有贡献边界、能过面试的口头答案。
```
